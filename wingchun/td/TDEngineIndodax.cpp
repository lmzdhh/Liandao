...
TDEngineIndodax::TDEngineIndodax(): ITDEngine(SOURCE_CTP),need_settleConfirm(true), need_authenticate(false), curAccountIdx(-1)
{
    logger = yijinjing::KfLog::getLogger("TradeEngine.Indodax");
    KF_LOG_INFO(logger, "[ATTENTION] default to confirm settlement and no authentication!");
}

void TDEngineIndodax::init()
{
    ITDEngine::init();
...
void TDEngineIndodax::resize_accounts(int account_num)
{
    account_units.resize(account_num);
}

TradeAccount TDEngineIndodax::load_account(int idx, const json& j_config)
{
    // internal load
...
void TDEngineIndodax::connect(long timeout_nsec)
{
...
void TDEngineIndodax::login(long timeout_nsec)
{
    for (int idx = 0; idx < account_units.size(); idx ++)
    {
        AccountUnitCTP& unit = account_units[idx];
...
void TDEngineIndodax::release_api()
{
...
bool TDEngineIndodax::is_logged_in() const
{
    for (auto& unit: account_units)
    {
...
bool TDEngineIndodax::is_connected() const
{
    for (auto& unit: account_units)
    {
        if (!unit.connected)
 ...
 * req functions
 */
void TDEngineIndodax::req_investor_position(const LFQryPositionField* data, int account_index, int requestId)
{
    //struct CThostFtdcQryInvestorPositionField req = parseTo(*data);
    KF_LOG_DEBUG(logger, "[req_pos]" << " (Bid)" << "Indodax"
                                     << " (Iid)" << "ldtest"
                                     << " (Tid)" << "btc_idr");

    account_units[account_index].api->getInfoC();
...
void TDEngineIndodax::req_qry_account(const LFQryAccountField *data, int account_index, int requestId)
{
...
void TDEngineIndodax::req_order_action(const LFOrderActionField* data, int account_index, int requestId, long rcv_time)
{
...
void TDEngineIndodax::OnRspAuthenticate(CThostFtdcRspAuthenticateField *pRspAuthenticateField,
...
void TDEngineIndodax::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo,
...
void TDEngineIndodax::OnRspSettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm,
                                             CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    if (pRspInfo != nullptr && pRspInfo->ErrorID != 0)
    {
...
void TDEngineIndodax::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout, CThostFtdcRspInfoField *pRspInfo,
                                  int nRequestID, bool bIsLast)
{
    if (pRspInfo != nullptr && pRspInfo->ErrorID == 0)
    {
...
void TDEngineIndodax::OnRspOrderInsert(CThostFtdcInputOrderField *pInputOrder, CThostFtdcRspInfoField *pRspInfo,
                                    int nRequestID, bool bIsLast)
{
...
void TDEngineIndodax::OnRspOrderAction(CThostFtdcInputOrderActionField *pInputOrderAction,
                                   CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    int errorId = (pRspInfo == nullptr) ? 0 : pRspInfo->ErrorID;
    const char* errorMsg = (pRspInfo == nullptr) ? nullptr : EngineUtil::gbkErrorMsg2utf8(pRspInfo->ErrorMsg);
...
void TDEngineIndodax::OnRspQryInvestorPosition(CThostFtdcInvestorPositionField *pInvestorPosition, CThostFtdcRspInfoField *pRspInfo,
                                     int nRequestID, bool bIsLast)
{
    int errorId = (pRspInfo == nullptr) ? 0 : pRspInfo->ErrorID;
    const char* errorMsg = (pRspInfo == nullptr) ? nullptr : EngineUtil::gbkErrorMsg2utf8(pRspInfo->ErrorMsg);
...
void TDEngineIndodax::OnRtnOrder(CThostFtdcOrderField *pOrder)
{
...
void TDEngineIndodax::OnRtnTrade(CThostFtdcTradeField *pTrade)
{
...
void TDEngineIndodax::OnRspQryTradingAccount(CThostFtdcTradingAccountField *pTradingAccount,
                                         CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    int errorId = (pRspInfo == nullptr) ? 0 : pRspInfo->ErrorID;
    const char* errorMsg = (pRspInfo == nullptr) ? nullptr : EngineUtil::gbkErrorMsg2utf8(pRspInfo->ErrorMsg);
...
BOOST_PYTHON_MODULE(libctptd)
{
    ...
    class_<TDEngineIndodax, boost::shared_ptr<TDEngineIndodax> >("Engine")
    .def(init<>())
    .def("init", &TDEngineIndodax::initialize)
    .def("start", &TDEngineIndodax::start)
    .def("stop", &TDEngineIndodax::stop)
    .def("logout", &TDEngineIndodax::logout)
    .def("wait_for_stop", &TDEngineIndodax::wait_for_stop);
}