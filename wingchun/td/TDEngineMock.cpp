...
void TDEngineMock::readWhiteLists(AccountUnitCoinmex& unit, const json& j_config)
{
...
bool TDEngineMock::hasSymbolInWhiteList(std::vector<SubscribeCoinmexBaseQuote> &sub, std::string symbol)
{
...
//example: btc_usdt
void TDEngineMock::split(std::string str, std::string token, SubscribeCoinmexBaseQuote& sub)
{
    if (str.size() > 0) {
...
void TDEngineMock::debug_print(std::vector<SubscribeCoinmexBaseQuote> &sub)
{
    int count = sub.size();
...
void TDEngineMock::debug_print(std::map<std::string, std::string> &keyIsStrategyCoinpairWhiteList)
{
    std::map<std::string, std::string>::iterator map_itr;
...
std::string TDEngineMock::getWhiteListCoinpairFrom(AccountUnitCoinmex& unit, const char_31 strategy_coinpair)
{
...
bool TDEngineMock::loadExchangeOrderFilters(AccountUnitCoinmex& unit, Document &doc)
{
...
void TDEngineMock::debug_print(std::map<std::string, SendOrderFilter> &sendOrderFilters)
{
...
SendOrderFilter TDEngineMock::getSendOrderFilter(AccountUnitCoinmex& unit, const char *symbol)
{
 ...
 * */
    ...
    if(errorId != 0)
    {
        raw_writer->write_error_frame(&pos, sizeof(LFRspPositionField), source_id, MSG_TYPE_LF_RSP_POS_MOCK, 1, requestId, errorId, errorMsg.c_str());
    }
}
...
int64_t TDEngineMock::fixPriceTickSize(int keepPrecision, int64_t price, bool isBuy) {
...
int TDEngineMock::Round(std::string tickSizeStr) {
...
void TDEngineMock::GetAndHandleOrderTradeResponse()
{
    //every account
    for (int idx = 0; idx < account_units.size(); idx ++)
    {
...
void TDEngineMock::moveNewtoPending(AccountUnitCoinmex& unit)
{
...
void TDEngineMock::retrieveOrderStatus(AccountUnitCoinmex& unit)
{
        ...
        if(d.HasMember("status"))
        {
                ...
                && rtn_order.VolumeTraded != orderStatusIterator->VolumeTraded))
            {
                ...
                raw_writer->write_frame(&rtn_order, sizeof(LFRtnOrderField),
                                        source_id, MSG_TYPE_LF_RTN_ORDER_MOCK,
                    ...
                    && rtn_order.VolumeTraded != orderStatusIterator->VolumeTraded))
                {
                    ...
                    strcpy(rtn_trade.ExchangeID, "mock");
                    ...
                    raw_writer->write_frame(&rtn_trade, sizeof(LFRtnTradeField),
                                            source_id, MSG_TYPE_LF_RTN_TRADE_MOCK, 1, -1);
                }
...
void TDEngineMock::addNewQueryOrdersAndTrades(AccountUnitCoinmex& unit, const char_31 InstrumentID,
...
std::vector<std::string> TDEngineMock::split(std::string str, std::string token)
{
...
void TDEngineMock::get_exchange_time(AccountUnitCoinmex& unit, Document& json)
{
...
void TDEngineMock::get_account(AccountUnitCoinmex& unit, Document& json)
{
...
void TDEngineMock::get_products(AccountUnitCoinmex& unit, Document& json)
{
 /*
[{
	"baseCurrency": "LTC",
...
void TDEngineMock::send_order(AccountUnitCoinmex& unit, const char *code,
...
void TDEngineMock::cancel_all_orders(AccountUnitCoinmex& unit, std::string code, Document& json)
{
...
void TDEngineMock::get_depth(AccountUnitCoinmex& unit, std::string code, Document& json)
{
...
void TDEngineMock::cancel_order(AccountUnitCoinmex& unit, std::string code, long orderId, Document& json)
{
...
//订单状态，﻿open（未成交）、filled（已完成）、canceled（已撤销）、cancel（撤销中）、partially-filled（部分成交）
void TDEngineMock::query_orders(AccountUnitCoinmex& unit, std::string code, std::string status, Document& json)
{
...
void TDEngineMock::query_order(AccountUnitCoinmex& unit, std::string code, long orderId, Document& json)
{
...
inline int64_t TDEngineMock::getTimestamp()
{
    long long timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    return timestamp;
}

std::string TDEngineMock::getTimestampString()
{
    long long timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
int64_t TDEngineMock::getTimeDiffOfExchange(AccountUnitCoinmex& unit)
{
