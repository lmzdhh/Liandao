...
TradeAccount TDEngineHitBTC::load_account(int idx, const json& j_config)
{
    ...
    if(unit.coinPairWhiteList.GetKeyIsStrategyCoinpairWhiteList().size() == 0) {
        KF_LOG_ERROR(logger, "TDEngineHitBTC::load_account: subscribeCoinmexBaseQuote is empty. please add whiteLists in kungfu.json like this :");
...
//当不出错时，返回结果信息
void TDEngineHitBTC::getResponse(int http_status_code, std::string responseText, std::string errorMsg, Document& json)
{
