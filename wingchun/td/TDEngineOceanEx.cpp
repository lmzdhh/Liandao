...
TradeAccount TDEngineOceanEx::load_account(int idx, const json& j_config)
{
    ...
    if(unit.coinPairWhiteList.GetKeyIsStrategyCoinpairWhiteList().size() == 0) {
        KF_LOG_ERROR(logger, "TDEngineOceanEx::load_account: subscribeCoinmexBaseQuote is empty. please add whiteLists in kungfu.json like this :");
...
void TDEngineOceanEx::connect(long timeout_nsec)
{
    ...
    KF_LOG_INFO(logger, "[connect] rest_thread start on TDEngineOceanEx::loop");
