...
USING_WC_NAMESPACE
using namespace oceanexb;
TDEngineOceanEx2::TDEngineOceanEx2(): ITDEngine(SOURCE_OCEANEXB)
{
    logger = yijinjing::KfLog::getLogger("TradeEngine.OCEANEXB");
    KF_LOG_INFO(logger, "[TDEngineOceanEx2]");
...
TDEngineOceanEx2::~TDEngineOceanEx2()
{
...
cpr::Response TDEngineOceanEx2::Post(const std::string& method_url,const std::string& body, AccountUnitOceanEx& unit)
{
...
void TDEngineOceanEx2::pre_load(const json& j_config)
{
    KF_LOG_INFO(logger, "[pre_load]");
}

void TDEngineOceanEx2::resize_accounts(int account_num)
{
    account_units.resize(account_num);
    KF_LOG_INFO(logger, "[resize_accounts]");
}

TradeAccount TDEngineOceanEx2::load_account(int idx, const json& j_config)
{
    ...
    if(unit.coinPairWhiteList.Size() == 0) {
        KF_LOG_ERROR(logger, "TDEngineOceanEx2::load_account: please add whiteLists in kungfu.json like this :");
...
void TDEngineOceanEx2::connect(long timeout_nsec)
{
...
void TDEngineOceanEx2::login(long timeout_nsec)
{
    KF_LOG_INFO(logger, "[login]");
    connect(timeout_nsec);
}

void TDEngineOceanEx2::logout()
{
    KF_LOG_INFO(logger, "[logout]");
}

void TDEngineOceanEx2::release_api()
{
    KF_LOG_INFO(logger, "[release_api]");
}

bool TDEngineOceanEx2::is_logged_in() const
{
...
bool TDEngineOceanEx2::is_connected() const
{
...
std::string TDEngineOceanEx2::GetSide(const LfDirectionType& input) {
...
LfDirectionType TDEngineOceanEx2::GetDirection(std::string input) {
    if ("buy" == input) {
        return LF_CHAR_Buy;
    } else if ("sell" == input) {
        return LF_CHAR_Sell;
...
std::string TDEngineOceanEx2::GetType(const LfOrderPriceTypeType& input) {
...
LfOrderPriceTypeType TDEngineOceanEx2::GetPriceType(std::string input) {
    if ("limit" == input) {
...
LfOrderStatusType TDEngineOceanEx2::GetOrderStatus(std::string input) {
    if ("wait" == input) {
 ...
 * req functions
 */
void TDEngineOceanEx2::req_investor_position(const LFQryPositionField* data, int account_index, int requestId)
{
    ...
    if(!d.HasParseError() && d.IsObject() && d.HasMember("code"))
    {
        ...
        if(errorId != 0) {
            ...
            KF_LOG_ERROR(logger, "[req_investor_position] failed!" << " (rid)" << requestId << " (errorId)" << errorId
                                                                   << " (errorMsg) " << errorMsg);
            raw_writer->write_error_frame(&pos, sizeof(LFRspPositionField), source_id, MSG_TYPE_LF_RSP_POS_OCEANEXB, 1, requestId, errorId, errorMsg.c_str());
        }
    }
    send_writer->write_frame(data, sizeof(LFQryPositionField), source_id, MSG_TYPE_LF_QRY_POS_OCEANEXB, 1, requestId);
...
void TDEngineOceanEx2::req_qry_account(const LFQryAccountField *data, int account_index, int requestId)
{
    KF_LOG_INFO(logger, "[req_qry_account]");
}


void TDEngineOceanEx2::req_order_insert(const LFInputOrderField* data, int account_index, int requestId, long rcv_time)
{
    AccountUnitOceanEx& unit = account_units[account_index];
    ...
    send_writer->write_frame(data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_OCEANEXB, 1/*ISLAST*/, requestId);
    ...
    if(ticker.length() == 0) {
        ...
        raw_writer->write_error_frame(data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_OCEANEXB, 1, requestId, errorId, errorMsg.c_str());
        return;
    }
  ...
  "message": "Operation is successful"
            ...
            raw_writer->write_frame(&rtn_order, sizeof(LFRtnOrderField),
                                    source_id, MSG_TYPE_LF_RTN_ORDER_OCEANEXB,
            ...
            raw_writer->write_error_frame(data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_OCEANEXB, 1,
...
void TDEngineOceanEx2::handlerResponsedOrderStatus(AccountUnitOceanEx& unit)
{
...
void TDEngineOceanEx2::req_order_action(const LFOrderActionField* data, int account_index, int requestId, long rcv_time)
{
    AccountUnitOceanEx& unit = account_units[account_index];
    ...
    send_writer->write_frame(data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_OCEANEXB, 1, requestId);
    ...
    if(ticker.length() == 0) {
        ...
        raw_writer->write_error_frame(data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_OCEANEXB, 1, requestId, errorId, errorMsg.c_str());
        return;
    }
...
void TDEngineOceanEx2::addRemoteOrderIdOrderActionSentTime(const LFOrderActionField* data, int requestId, int64_t remoteOrderId)
{
...
void TDEngineOceanEx2::GetAndHandleOrderTradeResponse()
{
    //every account
    for (size_t idx = 0; idx < account_units.size(); idx++)
    {
...
void TDEngineOceanEx2::retrieveOrderStatus(AccountUnitOceanEx& unit)
{
...
void TDEngineOceanEx2::addNewQueryOrdersAndTrades(AccountUnitOceanEx& unit, const char_31 InstrumentID,
...
void TDEngineOceanEx2::moveNewOrderStatusToPending(AccountUnitOceanEx& unit)
{
...
void TDEngineOceanEx2::set_reader_thread()
{
    ITDEngine::set_reader_thread();

    KF_LOG_INFO(logger, "[set_reader_thread] rest_thread start on TDEngineOceanEx2::loop");
    rest_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineOceanEx2::loop, this)));


    KF_LOG_INFO(logger, "[set_reader_thread] orderaction_timeout_thread start on TDEngineOceanEx2::loopOrderActionNoResponseTimeOut");
    orderaction_timeout_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineOceanEx2::loopOrderActionNoResponseTimeOut, this)));
}


void TDEngineOceanEx2::loop()
{
...
void TDEngineOceanEx2::loopOrderActionNoResponseTimeOut()
{
...
void TDEngineOceanEx2::orderActionNoResponseTimeOut()
{
...
void TDEngineOceanEx2::printResponse(const Document& d)
{
    StringBuffer buffer;
...
void TDEngineOceanEx2::getResponse(int http_status_code, std::string responseText, std::string errorMsg, Document& json)
{
...
std::string TDEngineOceanEx2::construct_request_body(const AccountUnitOceanEx& unit,const  std::string& data,bool isget)
{
...
void TDEngineOceanEx2::get_account(AccountUnitOceanEx& unit, Document& json)
{
 ...
 * */
std::string TDEngineOceanEx2::createInsertOrdertring(const char *code,
...
void TDEngineOceanEx2::send_order(AccountUnitOceanEx& unit, const char *code,
...
bool TDEngineOceanEx2::shouldRetry(Document& doc)
{
    bool ret = false;
...
void TDEngineOceanEx2::cancel_all_orders(AccountUnitOceanEx& unit, std::string code, Document& json)
{
...
void TDEngineOceanEx2::cancel_order(AccountUnitOceanEx& unit, std::string code, std::string orderId, Document& json)
{
...
void TDEngineOceanEx2::query_order(AccountUnitOceanEx& unit, std::string code, std::string orderId, Document& json)
{
...
void TDEngineOceanEx2::handlerResponseOrderStatus(AccountUnitOceanEx& unit, std::vector<PendingOrderStatus>::iterator orderStatusIterator, ResponsedOrderStatus& responsedOrderStatus)
{
    ...
    else
    {
            ...
            && rtn_order.VolumeTraded != orderStatusIterator->VolumeTraded))
        {
            ...
            //calculate the volumn and price (it is average too)
            rtn_trade.Volume = rtn_order.VolumeTraded - orderStatusIterator->VolumeTraded;
            rtn_trade.Price = std::llround((newAmount - oldAmount)*1.0/(rtn_trade.Volume)+0.1);
            strncpy(rtn_trade.OrderSysID,strOrderID.c_str(),31);
            on_rtn_trade(&rtn_trade);
            raw_writer->write_frame(&rtn_trade, sizeof(LFRtnTradeField),
                                    source_id, MSG_TYPE_LF_RTN_TRADE_OCEANEXB, 1, -1);
...
std::string TDEngineOceanEx2::parseJsonToString(Document &d)
{
    StringBuffer buffer;
    Writer<StringBuffer> writer(buffer);
    d.Accept(writer);
...
inline int64_t TDEngineOceanEx2::getTimestamp()
{
...
BOOST_PYTHON_MODULE(liboceanexbtd)
{
    using namespace boost::python;
    class_<TDEngineOceanEx2, boost::shared_ptr<TDEngineOceanEx2> >("Engine")
            .def(init<>())
            .def("init", &TDEngineOceanEx2::initialize)
            .def("start", &TDEngineOceanEx2::start)
            .def("stop", &TDEngineOceanEx2::stop)
            .def("logout", &TDEngineOceanEx2::logout)
            .def("wait_for_stop", &TDEngineOceanEx2::wait_for_stop);
}
