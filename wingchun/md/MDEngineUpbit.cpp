...
void MDEngineUpbit::load(const json& j_config)
{
    ...
    KF_LOG_INFO(logger, "MDEngineUpbit::load:" << "book_depth_count=" << book_depth_count <<
...
void MDEngineUpbit::login(long timeout_nsec) {
    ...
    char inputURL[300] = "wss://demo-api.upbit.com/api/exchange/v1/ws";
...
void MDEngineUpbit::set_reader_thread()
{
    ...
    rest_thread = ThreadPtr(new std::thread(boost::bind(&MDEngineUpbit::loop, this)));
}
...
int MDEngineUpbit::lws_write_subscribe(struct lws* conn)
{
    ...
    {
        //subscribe_index = 0;
        KF_LOG_INFO(logger, "MDEngineUpbit::lws_write_subscribe: (none reset subscribe_index = 0, just return 0)");
	    return 0;
    }
    ...
    KF_LOG_INFO(logger, "MDEngineUpbit::lws_write_subscribe: " << jsonString.c_str() << " ,after ++, (subscribe_index)" << subscribe_index);
    ...
    if(subscribe_index < websocketSubscribeJsonString.size())
    {
        ...
        KF_LOG_INFO(logger, "MDEngineUpbit::lws_write_subscribe: (subscribe_index < websocketSubscribeJsonString) call lws_callback_on_writable");
    }
...
void MDEngineUpbit::on_lws_data(struct lws* conn, const char* data, size_t len)
{
    ...
    if(!json.HasParseError() && json.IsObject() && json.HasMember("channel") && json["channel"].IsString())
    {
        ...
        {
            KF_LOG_INFO(logger, "MDEngineUpbit::on_lws_data: is marketdata");
            onMarketData(json);
        }
    } else {
...
**/
void MDEngineUpbit::onMarketData(Document& json)
{
    std::string channel="";
    ...
    if(json.HasMember("reset") && json["reset"].IsBool()) 
    {
        ...
        KF_LOG_INFO(logger, "MDEngineUpbit::onMarketData:" << "reset : " << reset);
    }
    ...
    KF_LOG_INFO(logger, "MDEngineUpbit::onMarketData:" << "channel : " << channel <<
                                                            "  market_id: " << market_id << 
                                                            "  status: " << status << 
                                                            "  lag: " << lag << 
                                                            "  ticker: " << ticker);
    ...
    if(json.HasMember("ticker") && json["ticker"].IsObject()) 
    {
        ...
        KF_LOG_INFO(logger, "MDEngineUpbit::onMarketData:" << "ticker_time : " << ticker_time << "  ticker_last : " << ticker_last << "  ticker_low : " << ticker_low << "  ticker_high : " << ticker_high << "  ticker_change : " << ticker_change << "  ticker_base_volume : " << ticker_base_volume << "  ticker_quote_volume : " << ticker_quote_volume);
    }
	if(json.HasMember("recent_trades") && json["recent_trades"].IsArray()) 
	{
        ...
        for(int i = 0 ; i < len; i++)
        {
            ...
            KF_LOG_INFO(logger, "MDEngineUpbit::onMarketData:" << "rt_side : " << rt_side << "  rt_price: " << rt_price << "  rt_quantity: " << rt_quantity << "  rt_tick_direction: " << rt_tick_direction << "  rt_time: " << rt_time);
        }
    }
    */
	...
	if(json.HasMember("order_books") && json["order_books"].IsArray()) 
	{
        ...
        for(int i = 0 ; i < len; i++)
        {
            ...
            KF_LOG_INFO(logger, "MDEngineUpbit::onMarketData:" << "ob_side : " << ob_side << "  ob_price: " << ob_price << "  ob_quantity: " << ob_quantity);
        }
    }
    ...
    if(priceBook20Assembler.Assembler(ticker, md)) 
    {
        ...
        KF_LOG_INFO(logger, "MDEngineUpbit::onMarketData: on_price_book_update");
 ...
 * */
std::string MDEngineUpbit::createMarketDataJsonString(std::string base_quote)
{
    StringBuffer s;
