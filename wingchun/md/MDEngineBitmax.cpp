...
void MDEngineBitmax::load(const json& j_config)
{
    ...
    KF_LOG_INFO(logger, "MDEngineBitmax:: rest_get_interval_ms: " << rest_get_interval_ms);
...
void MDEngineBitmax::readWhiteLists(const json& j_config)
{
...
//example: btc_usdt
void MDEngineBitmax::split(std::string str, std::string token, SubscribeCoinBaseQuote& sub)
{
	if (str.size() > 0) {
...
std::string MDEngineBitmax::getWhiteListCoinpairFrom(std::string md_coinpair)
{
...
void MDEngineBitmax::debug_print(std::vector<SubscribeCoinBaseQuote> &sub)
{
    int count = sub.size();
...
void MDEngineBitmax::debug_print(std::map<std::string, std::string> &keyIsStrategyCoinpairWhiteList)
{
	std::map<std::string, std::string>::iterator map_itr;
	map_itr = keyIsStrategyCoinpairWhiteList.begin();
	while(map_itr != keyIsStrategyCoinpairWhiteList.end()) {
...
void MDEngineBitmax::debug_print(std::vector<std::string> &subJsonString)
{
...
void MDEngineBitmax::login(long timeout_nsec)
{
	...
	{
		KF_LOG_ERROR(logger, "MDEngineBitmax::connect: Couldn't parse URL. Please check the URL and retry: " << inputURL);
		return;
	}
	...
	KF_LOG_INFO(logger, "MDEngineBitmax::login:" << "urlProtocol=" << urlProtocol <<
	...
	KF_LOG_INFO(logger, "MDEngineBitmax::login: context created.");


	if (context == NULL) {
		KF_LOG_ERROR(logger, "MDEngineBitmax::login: context is NULL. return");
		return;
	}
	...
	KF_LOG_INFO(logger, "MDEngineBitmax::login:" << "Connecting to " << urlProtocol << ":" <<
												  clientConnectInfo.host << ":" <<
												  clientConnectInfo.port << ":" << urlPath);

	wsi = lws_client_connect_via_info(&clientConnectInfo);
	if (wsi == NULL) {
		KF_LOG_ERROR(logger, "MDEngineBitmax::login: wsi create error.");
		return;
	}
	KF_LOG_INFO(logger, "MDEngineBitmax::login: wsi create success.");
	logged_in = true;
}
...
{
	KF_LOG_INFO(logger, "MDEngineBitmax::lws_write_subscribe: (subscribe_index)" << subscribe_index);
    ...
    KF_LOG_INFO(logger, "MDEngineBitmax::lws_write_subscribe: " << jsonString.c_str());
...
void MDEngineBitmax::on_lws_data(struct lws* conn, const char* data, size_t len)
{
	...
	if(!json.HasParseError() && json.IsObject() && json.HasMember("type") && json["type"].IsString())
	{
		...
		{
			KF_LOG_INFO(logger, "MDEngineBitmax::on_lws_data: is depth");
            onDepth(json);
		}

		if(strcmp(json["type"].GetString(), "fills") == 0)
		{
			KF_LOG_INFO(logger, "MDEngineBitmax::on_lws_data: is fills");
            onFills(json);
		}
        if(strcmp(json["type"].GetString(), "tickers") == 0)
        {
            KF_LOG_INFO(logger, "MDEngineBitmax::on_lws_data: is tickers");
            onTickers(json);
        }
	} else {
...
{
	KF_LOG_ERROR(logger, "MDEngineBitmax::on_lws_connection_error.");
	//market logged_in false;
    logged_in = false;
    KF_LOG_ERROR(logger, "MDEngineBitmax::on_lws_connection_error. login again.");
...
void MDEngineBitmax::clearPriceBook()
{
...
void MDEngineBitmax::onTickers(Document& d)
{
/*
{
    "type": "tickers",
...
void MDEngineBitmax::onFills(Document& json)
{
    /*
     {
  "base": "cel",
  ...
  "type": "fills"
    ...
    {
        KF_LOG_ERROR(logger, "MDEngineBitmax::[onFills] invalid market trade message");
        return;
    }
    ...
    KF_LOG_INFO(logger, "MDEngineBitmax::onFills:" << "base : " << base << "  quote: " << quote);
    ...
    for(int i = 0 ; i < len; i++) {
        ...
        KF_LOG_INFO(logger, "MDEngineBitmax::[onFills] (ticker)" << ticker <<
                                                                    " (Price)" << trade.Price <<
...
void MDEngineBitmax::onDepth(Document& json)
{
    ...
    KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:" << "base : " << base << "  quote: " << quote);

    std::string ticker = getWhiteListCoinpairFrom(base + "_" +  quote);
    if(ticker.length() == 0) {
		KF_LOG_INFO(logger, "MDEngineBitmax::onDepth: not in WhiteList , ignore it:" << "base : " << base << "  quote: " << quote);
		return;
    }

    KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:" << "(ticker) " << ticker);
...
//        KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:" << "ticker : " << ticker << "  get from map (asksPriceAndVolume.size) " << asksPriceAndVolume->size());
	} else {
...
//        KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:" << "ticker : " << ticker << "  insert into map (asksPriceAndVolume.size) " << asksPriceAndVolume->size());
	}

	iter = tickerBidPriceMap.find(ticker);
	if(iter != tickerBidPriceMap.end()) {
		bidsPriceAndVolume = iter->second;
//        KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:" << "ticker : " << ticker << "  get from map (bidsPriceAndVolume.size) " << bidsPriceAndVolume->size());
	} else {
...
//        KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:" << "ticker : " << ticker << "  insert into map (bidsPriceAndVolume.size) " << bidsPriceAndVolume->size());
	}
        ...
        if(json["data"].HasMember("asks") && json["data"]["asks"].IsArray()) {
            ...
            for(int i = 0 ; i < len; i++)
            {
                ...
                if(volume == 0) {
                    ...
                    KF_LOG_INFO(logger, "MDEngineBitmax::onDepth: ##########################################asksPriceAndVolume volume == 0############################# price:" << price<<  "  volume:"<< volume);
                } else {
                    asksPriceAndVolume->insert(std::pair<int64_t, uint64_t>(price, volume));
                }
//                KF_LOG_INFO(logger, "MDEngineBitmax::onDepth: asks price:" << price<<  "  volume:"<< volume);
                asks_update = true;
            }
        }
        ...
        if(json["data"].HasMember("bids") && json["data"]["bids"].IsArray()) {
            ...
            for(int i = 0 ; i < len; i++)
            {
                ...
                if(volume == 0) {
                    ...
                    KF_LOG_INFO(logger, "MDEngineBitmax::onDepth: ##########################################bidsPriceAndVolume volume == 0############################# price:" << price<<  "  volume:"<< volume);

                } else {
                    bidsPriceAndVolume->insert(std::pair<int64_t, uint64_t>(price, volume));
                }
//                KF_LOG_INFO(logger, "MDEngineBitmax::onDepth: bids price:" << price<<  "  volume:"<< volume);
                bids_update = true;
            }
        }
    }
    ...
    if(asks_update || bids_update)
    {
        ...
        for(int i = 0; i < size; ++i)
        {
            ...
            KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:  LFPriceBook20Field AskLevels: (i)" << i << "(price)" << md.AskLevels[i].price<<  "  (volume)"<< md.AskLevels[i].volume);
        }
        ...
        for(int i = 0; i < size; ++i)
        {
            ...
            KF_LOG_INFO(logger, "MDEngineBitmax::onDepth:  LFPriceBook20Field BidLevels: (i) " << i << "(price)" << md.BidLevels[i].price<<  "  (volume)"<< md.BidLevels[i].volume);
        }
 ...
 * */
std::string MDEngineBitmax::createDepthJsonString(std::string base, std::string quote)
{
    /*
{
    "event":"subscribe",
...
std::string MDEngineBitmax::createFillsJsonString(std::string base, std::string quote)
{
    /*
 {
"event": "subscribe",
...
std::string MDEngineBitmax::createTickersJsonString()
{
	StringBuffer s;
	Writer<StringBuffer> writer(s);
	writer.StartObject();
