...
def initialize(context):
    #context.add_md(source=SOURCE.KUCOIN)
    context.ticker = 'eth_btc'
    context.exchange_id = EXCHANGE.SHFE
    context.buy_price = 10000
    context.sell_price = 9999999999
    context.order_rid = -1
    context.cancel_id = -1
    context.add_td(source=SOURCE.KUCOIN)
    context.subscribe(tickers=[context.ticker], source=SOURCE.KUCOIN)
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.req_pos(source=SOURCE.KUCOIN)
            ...
            if context.order_rid < 0:
                ...
                context.order_rid = context.insert_limit_order(source=SOURCE.KUCOIN,
...
def on_rtn_order(context, rtn_order, order_id, source, rcv_time):
    ...
    if order_id == context.order_rid and rtn_order.OrderStatus == '5':
        ...
        context.stop()
    print('order_id',order_id,'status',rtn_order.OrderStatus)
