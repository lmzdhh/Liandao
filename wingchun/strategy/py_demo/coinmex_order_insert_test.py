...
def initialize(context):
    ...
    context.ticker = 'TRXBTC'
    context.exchange_id = EXCHANGE.SHFE
    context.buy_price = -1
    context.sell_price = -1
    context.order_rid = -1
    context.cancel_id = -1
    context.add_td(source=SOURCE.COINMEX)
    context.subscribe(tickers=[context.ticker], source=SOURCE.COINMEX)
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.req_pos(source=SOURCE.COINMEX)
            ...
            #context.stop()
            print '----will test buy ----'
            context.buy_price = 000000001 #market_data.LowerLimitPrice
