...
def initialize(context):
    #context.add_md(source=SOURCE.BITMEX)
    context.ticker = 'btc_usdt'
    context.exchange_id = EXCHANGE.BITMEX
    context.buy_price = -1
    context.sell_price = -1
    context.order_rid = -1
    context.cancel_id = -1
    context.add_td(source=SOURCE.BITMEX)
    context.subscribe(tickers=[context.ticker], source=SOURCE.BITMEX)
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.req_pos(source=SOURCE.BITMEX)
            ...
            context.buy_price = 1191234 #market_data.LowerLimitPrice
