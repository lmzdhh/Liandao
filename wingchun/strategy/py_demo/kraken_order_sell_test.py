...
def initialize(context):
    context.add_md(source=SOURCE.KRAKEN)
    context.ticker = 'usdt_usd'
    context.exchange_id = EXCHANGE.SHFE
    context.buy_price = -1
    context.sell_price = -1
    context.order_rid = -1
    context.cancel_id = -1
    context.add_td(source=SOURCE.KRAKEN)
    context.subscribe(tickers=[context.ticker], source=SOURCE.KRAKEN)
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.req_pos(source=SOURCE.KRAKEN)
            ...
            #context.stop()
            context.buy_price = 98650000 #market_data.LowerLimitPrice
            context.sell_price = 99999999 #market_data.UpperLimitPrice
            if context.order_rid < 0:
                print("context.insert_limit_order 512.")
                context.order_rid = context.insert_limit_order(source=SOURCE.KRAKEN,
