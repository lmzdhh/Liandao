...
wingchun strategy -n YOUR_STRATEGY1 -p hitbtc_order_cancel_test.py
'''

def initialize(context):
    #context.add_md(source=SOURCE.HITBTC)
    ...
    context.order_rid = -1
    context.cancel_id = -1
    context.add_td(source=SOURCE.HITBTC)
    context.subscribe(tickers=[context.ticker], source=SOURCE.HITBTC)
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.buy_price = 161470 #market_data.LowerLimitPrice
            context.sell_price = 999999999 #market_data.UpperLimitPrice
            if context.order_rid < 0:
                print("context.insert_limit_order 1.")
                context.order_rid = context.insert_limit_order(source=SOURCE.HITBTC,
