...
def initialize(context):
   #context.add_md(source=SOURCE.POLONIEX)
    ...
    context.order_rid = -1
    context.cancel_id = -1
    context.add_td(source=SOURCE.POLONIEX)
    #context.subscribe(tickers=[context.ticker], source=SOURCE.POLONIEX)
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.req_pos(source=SOURCE.POLONIEX)
            print 'req pos ,,,,,'
            ...
            context.buy_price = 709558305026 #market_data.LowerLimitPrice
            ...
            if context.order_rid < 0:
                ...
                print('will cancel it after 15s')
                import time
                time.sleep(15)
                print('canceling...')
