...
//wss://api.daybit.com/v1/user_api_socket/websocket/

static std::string host  = "api.daybit.com";
static std::string path = "/v1/user_api_socket/websocket?api_key=M0f-KWvkb1y28wv-nT-4vwPBnomZMNrt&api_secret=Hbj4zKZiwbXQB25Te39TwDuqbo9oiicY";
static int port = 443;
...
//static std::string jsonString = "{\"join_ref\": \"1\", \"ref\": \"1\", \"topic\": \"/subscription:order_books;USDT;BTC;0.20000000\", \"event\": \"phx_join\", \"payload\": {}, \"timeout\": 3000}";

//static std::string jsonString1 = "{\"join_ref\": \"1\", \"ref\": \"2\", \"topic\": \"/subscription:order_books;USDT;BTC;0.20000000\", \"event\": \"request\", \"payload\": {\"timestamp\": %s}, \"timeout\": 3000}";

//static std::string jsonString ="{\"join_ref\": \"1\", \"ref\": \"1\", \"topic\": \"/subscription:trades;USDT;BTC\", \"event\": \"phx_join\", \"payload\": {}, \"timeout\": 3000}";
//static std::string jsonString1 ="{\"join_ref\": \"1\", \"ref\": \"2\", \"topic\": \"/subscription:trades;USDT;BTC\", \"event\": \"request\", \"payload\": {\"size\": 10, \"timestamp\": %s}, \"timeout\": 3000}";

static std::string jsonString = "{\"join_ref\": \"1\", \"ref\": \"1\", \"topic\": \"/subscription:markets\", \"event\": \"phx_join\", \"payload\": {}, \"timeout\": 3000}";;
static std::string jsonString1 = "{\"join_ref\": \"1\", \"ref\": \"2\", \"topic\": \"/subscription:markets\", \"event\": \"request\", \"payload\": {\"timestamp\": %s}, \"timeout\": 3000}";
...
static int lws_write_subscribe(struct lws* conn)
{
	...
	if (sub ==0) {
		...
		char json[512] = {0};
		sprintf(json, jsonString1.c_str(), getTimestampString().c_str());
		std::cout << "lws_write_subscribe: " << json << std::endl;
    	length = strlen(json);
    	strncpy((char *)msg+LWS_PRE, json, length);
		sub = 2;
	} else {
		 sub++;
		 if (sub == 50) sub =1;
